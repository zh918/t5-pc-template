...
export default {
  data() {
    return {
      ...
      operatorContainer: [{ type: 'primary', text: '新增系统', cb: this.handleAddSystem}],
      tableContainer: {
        selection: true,
        selectionChange: () => {},
        operate: {
      ...
      rules: {
        NAME: [{ type: 'string', required: true, message: '系统名称不能为空'}],
        BEGIN_TIME: [{ type: 'string', required: true, message: '有效期开始不能为空'}],
        END_TIME: [{ type: 'string', required: true, message: '有效期结束不能为空'}],
        STATUS: [{ type: 'string', required: true, message: '系统状态不能为空'}]
      },
      flag: {
